...
def gen_params(cfg, params, out):
    ...
    lines = ["/* generated by mpc-vst-plugins tools/gen_vst.py from vst.json + module.json: do not edit */",
             ...
             "const char *const *opts; int momentary; } param_t;"]
    rows = []
    for i, p in enumerate(params):
        ...
        if opts:
            ...
            rows.append("    {%s, %s, \"\", 0, 0, %s, %d, OPTS_%d, %d}," % (
                c_str(p["key"]), name, fl(norm), len(opts), i, p.get("access") == "write"))
        else:
            lo, hi = p.get("min", 0), p.get("max", 1)
            d = p.get("default", lo)
            norm = (d - lo) / (hi - lo) if hi > lo else 0
            rows.append("    {%s, %s, %s, %s, %s, %s, 0, 0, %d}," % (
