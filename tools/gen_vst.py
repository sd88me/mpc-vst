...
def gen_params(cfg, params, out):
    ...
    lines = ["/* generated by mpc-vst-plugins tools/gen_vst.py from vst.json + module.json: do not edit */",
             ...
             "const char *const *opts; int momentary; int string_display; int int_display; } param_t;"]
    rows = []
    for i, p in enumerate(params):
        ...
        if opts:
            ...
            rows.append("    {%s, %s, \"\", 0, 0, %s, %d, OPTS_%d, %d, %d, %d}," % (
            ...
            rows.append("    {%s, %s, %s, %s, %s, %s, 0, 0, %d, %d, %d}," % (
                c_str(p["key"]), name, c_str(p.get("unit", "")), fl(lo), fl(hi), fl(norm),
                p.get("access") == "write", is_str, is_int))
