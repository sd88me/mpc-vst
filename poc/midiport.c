...
static intptr_t dispatcher(AEffect *e, int32_t op, int32_t idx, intptr_t v, void *p, float o) {
    ...
    switch (op) {
    case 35: return 2;
    case 45: case 48: strcpy(p, "MPC MIDI Port PoC"); return 1;
...
static void processReplacing(AEffect *e, float **in, float **o, int32_t n) {
    ...
    for (int i = 0; i < out.numEvents && seq; i++) {
        const unsigned char *m = pool[i].midiData;
        snd_seq_event_t ev;
        snd_seq_ev_clear(&ev);
        snd_seq_ev_set_source(&ev, seq_port);
        snd_seq_ev_set_subs(&ev);
        snd_seq_ev_set_direct(&ev);
        if ((m[0] & 0xf0) == 0x90 && m[2]) snd_seq_ev_set_noteon(&ev, m[0] & 15, m[1], m[2]);
        else if ((m[0] & 0xf0) == 0x80 || (m[0] & 0xf0) == 0x90) snd_seq_ev_set_noteoff(&ev, m[0] & 15, m[1], 0);
        else continue;
        int r = snd_seq_event_output_direct(seq, &ev);
        if (r >= 0) sent_ok++; else sent_fail++;
        if (sent_ok + sent_fail <= 5 || (sent_ok + sent_fail) % 100 == 0)
            LOG("alsa send %02x %d -> %d (ok=%ld fail=%ld)\n", m[0], m[1], r, sent_ok, sent_fail);
    }
...
__attribute__((visibility("default"))) AEffect *VSTPluginMain(audioMasterCallback m) {
    master = m;
    if (!lg) lg = fopen("/tmp/midiport.log", "a");
    LOG("VSTPluginMain\n");
